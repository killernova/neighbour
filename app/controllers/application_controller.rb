...
class ApplicationController < ActionController::Base
  ...
  def auto_login
    ...
    session[:locale] = 'zh' if current_user.nil?
  end
  ...
  def create_sign(hash)
    key = WX_API_KEY
    stringA = hash.select { |_key, value| value.present? }.sort.map do |arr|
      arr.map(&:to_s).join('=')
    end
    stringA = stringA.join('&')
    string_sing_temp = stringA + "&key=#{key}"
    (Digest::MD5.hexdigest string_sing_temp).upcase
  end
